...
{\large Cao học khoá K35 -- Ngành Khoa học dữ liệu\par}
\vspace{1.6cm}

{\huge\bfseries Trợ lý chẩn đoán nguyên nhân gốc rễ \\[0.3em]
sự cố mạng/CNTT dựa trên tri thức: \\[0.3em]
Knowledge Graph, luật suy diễn \\[0.3em]
và GraphRAG\par}

\vspace{2.2cm}

{\large Học viên thực hiện: \textbf{Khang}\par}
...
\paragraph{Về mặt biểu diễn: luật Horn-clause, nhưng cài đặt thủ tục hoá.} Về mặt lý thuyết, mỗi luật trong
Bảng~\ref{tab:rules} có đúng dạng một \textbf{Horn clause} (\(P_1 \wedge P_2 \wedge \dots \Rightarrow Q\)),
hình thức suy diễn dựa luật kinh điển. Tuy nhiên, khác với một hệ chuyên gia "thuần" (vd. cài trên nền
Prolog hoặc CLIPS/Drools) -- nơi tập luật được lưu như \emph{dữ liệu khai báo}, tách biệt khỏi một bộ
suy diễn tổng quát (generic inference engine) thực hiện forward/backward chaining -- bốn luật ở đây được
\textbf{viết cứng (hard-coded) trực tiếp thành hàm Python} (Đoạn mã~\ref{lst:ruleA}), không lưu trong
Neo4j như dữ liệu (khác với Device/Alarm/KPI -- những thực thể đó \emph{là} node/edge thật trong đồ thị).
Nói cách khác, đây là một dạng \textbf{thủ tục hoá (procedural encoding)} của logic Horn-clause: hợp lý
cho quy mô prototype (không cần tự viết một bộ suy diễn tổng quát), nhưng có đánh đổi rõ ràng so với một
rule base khai báo thật sự -- thêm/sửa luật đòi hỏi sửa code Python thay vì chỉ khai báo lại dữ liệu luật.
